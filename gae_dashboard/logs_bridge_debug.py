...
parse through if you are investigating a single alert and are trying ot get to
...
def main(config_filename, metric_name, start_time_t,
    ...
    config = logs_bridge.load_config(config_filename)
    match = next((c for c in config if c['metricName'] == metric_name), None)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--metric', required=True,
        help=('A single metricName to examine from the '
              'configuration file (ex: "logs.status.400.week_over_week")'))

    parser.add_argument('--start_time', type=int, required=True,
        help=('Start time to examine, specified as seconds since'
              ' unix epoch (ex: `gdate -d "3am today" +%%s`)'))

    parser.add_argument('-c', '--config', default='logs_bridge.config.json',
        help=('JSON file holding the stats to extract from the logs'))
    args = parser.parse_args()

    logs_format = '[%(levelname)s] %(message)s'
    logging.basicConfig(format=logs_format, level=logging.INFO)

    main(args.config, args.metric, args.start_time)
